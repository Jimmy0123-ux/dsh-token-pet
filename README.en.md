...
> **Version 0.4.2:** the panel no longer reports deleted/archived sessions as read failures — when a session the host still lists has no log left, the ledger keeps its retained values and stays quiet instead of showing "N sessions could not be read". Stale live flags that caused endless re-reads are cleared too; genuine read errors still surface. 0.4.1 fixed a settings-merge bug where a partial settings event reset unrelated preferences, and added the interface screenshots and this rewritten guide. 0.4.0 brought dark/light theme switching, a visual price editor, cost estimates now **off by default** (opt-in), plus fixes for unreadable button labels caused by the host's dark-mode CSS and for the client bundle failing to load when the skin ZIP importer pulled in fflate.
...
```sh
dsh plugin --profile desktop add dsh-token-pet@0.4.2
dsh plugin --profile web add dsh-token-pet@0.4.2
```
