...
当前发布版本：`0.3.0` · Node.js `>=22.19`

> 本版本新增成本估算与月预算、近 7/30 日趋势、会话排行、用量数据导出、皮肤 ZIP 客户端导入与 3 套新配色皮肤、提示音主题与音量。此前版本新增完整中英双语界面、响应式浮窗布局、默认关闭的完成提示音、默认 Prompt Optimizer 规则及连续编辑增强流程。
...
- **成本估算与月预算**：按本地价格表估算本月与各模型成本，可选月预算超限预警动作与提示音；
...
| 成本估算 | 浮窗“总览”成本卡片：本月成本与月预算进度；模型页每模型估算成本 |
...
- 纯本地价格表（USD / 1M token），键支持模型名、`claude-*` 前缀与 `*` 兜底，可在设置中编辑或恢复默认；
- 概览展示本月估算成本与预算进度；模型页展示每个模型的估算成本（模型 Top5 也附带成本列）；
